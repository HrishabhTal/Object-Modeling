...
public class ContestRepository implements IContestRepository {
    ...
    public List<Contest> findAll() {
     List<Contest> c=contestMap.entrySet().stream().map(Map.Entry::getValue).collect(Collectors.toList());
     return c;
    }
    ...
    public List<Contest> findAllContestLevelWise(Level level) {
     ...
     List<Contest> l=contestMap.entrySet().stream().filter(e->e.getValue().getLevel().equals(level)).map(Map.Entry::getValue).collect(Collectors.toList());
     return l;
    }
